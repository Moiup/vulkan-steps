...
A raw example of the use of compute pipline / shader alone (without the graphics pipeline).

It creates an input buffer with 10 values set to 10. The shader take this buffer and store the result of the sum of the first buffer value + the thread id in a second output buffer. Both buffers are storage buffers.

File (shader): [compute.comp](../Code/shaders/compute.comp)
File (C++):    [compute_only.cpp](../Code/examples/compute_only.cpp)

## Output
The programm will display the different layers properties available on your computer, as well as the different physical device and the selected physical device. At the end of its executiont, le program will display the result of the sum like so:
```
