...
int main()
{
	...
	/**************/
	// Semaphore
	for (auto sem : acquired_image_sem_arr) {
	...
	for (auto sem : compute_start_sem_arr) {
		vkDestroySemaphore(logical_device, sem, nullptr);
	}
	for (auto sem : compute_end_sem_arr) {
		vkDestroySemaphore(logical_device, sem, nullptr);
	}
	// Fence
	...
	// Pipeline
	for (auto compute_pipeline : compute_pipelines_arr) {
	...
	vkFreeDescriptorSets(
		logical_device,
		compute_descriptor_pool,
		compute_descriptor_sets_info.descriptorSetCount,
		compute_descriptor_sets_arr.data()
	);
	// Descriptor pool
